...
from app.schema import  ResponseSchema,VlanCreate
from app.repository.vlan import VlanRepository
...
async def create(current_user : Annotated[schema.TokenData, Depends(outh2.get_current_user)],create_form: VlanCreate):
    if current_user.user_type != 1:
        raise HTTPException(detail="U are not allowed to access this route",status_code=status.HTTP_401_UNAUTHORIZED)
    await VlanRepository.create(create_form=create_form)
...
async def update(current_user : Annotated[schema.TokenData, Depends(outh2.get_current_user)],id: Annotated[int,Path(title="Vlan ID",ge=1)], update_form: VlanCreate):
    if current_user.user_type != 1:
        raise HTTPException(detail="U are not allowed to access this route",status_code=status.HTTP_401_UNAUTHORIZED)
    await VlanRepository.update(vlan_id=id,update_form=update_form)
...
async def delete(current_user : Annotated[schema.TokenData, Depends(outh2.get_current_user)],id: Annotated[int,Path(title="Vlan ID",ge=1)]):
    if current_user.user_type != 1:
        raise HTTPException(detail="U are not allowed to access this route",status_code=status.HTTP_401_UNAUTHORIZED)
    await VlanRepository.delete(vlan_id=id)
...
async def get_by_site_id(
    ...
    result = await VlanRepository.get_all_by_site_id(site_id=site_id,page=page,limit=limit,
